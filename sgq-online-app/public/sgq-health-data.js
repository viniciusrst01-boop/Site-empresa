...
function healthObservation(source, value, now = new Date()) {
  ...
  return { key: `${HEALTH_PREFIX}${source}:${healthMonth(now)}`, value: { source, recordedAt: new Date(now).toISOString(), ...counts } };
}
...
function getSGQHealthHistory(dataRows, { months = 6, canView = () => true, now = new Date() } = {}) {
  ...
  const atMonth = (key, current = false) => {
    const values = {};
    for (const [metric, [source, module]] of Object.entries(metricSource)) {
      ...
      const observation = current
        ...
        : observations[source].filter((item) => healthMonth(item.recordedAt) <= key).at(-1);
  ...
  const current = atMonth(endMonth, true);
  const points = Array.from({ length: months }, (_, index) => {
    const date = new Date(Date.UTC(year, month - months + index, 15));
    const key = date.toISOString().slice(0, 7);
    return { month: key, ...atMonth(key, key === endMonth) };
  });
  return { months, current, points, hasData: Object.entries(metricSource).some(([, [source, module]]) => canView(module) && (Boolean(currentValues[source]) || observations[source].length > 0)) };
}
