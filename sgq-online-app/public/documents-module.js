...
function documentEffectiveStatus(row) {
  ...
  return documentIsLate(row) ? "Revisar documento" : row.status;
}
...
function renderDocumentsCharts() {
  ...
  const make = (id, type, entries) => { const canvas = document.getElementById(id); if (!canvas) return; const doughnut = type === "doughnut"; documentsCharts[id] = new Chart(canvas, { type, data: { labels: entries.map(([label]) => label), datasets: [{ data: entries.map(([, value]) => value), backgroundColor: colors, borderColor: "#0b2d50", borderWidth: 2, borderRadius: type === "bar" ? 5 : 0 }] }, options: { responsive: true, maintainAspectRatio: false, layout: doughnut ? { padding: { left: 12 } } : {}, plugins: { legend: doughnut ? { position: "right", align: "center", labels: { color: "#9db8d2", boxWidth: 11, boxHeight: 11, usePointStyle: true, pointStyle: "circle", padding: 14, font: { size: 10 } } } : { position: "bottom", labels: { color: "#9db8d2", boxWidth: 11, font: { size: 10 } } } }, scales: type === "bar" ? { x: { ticks: { color: "#9db8d2", font: { size: 10 } }, grid: { color: "rgba(157,184,210,.12)" } }, y: { beginAtZero: true, ticks: { precision: 0, color: "#9db8d2" }, grid: { color: "rgba(157,184,210,.12)" } } } : {} } }); };
...
function documentEditorFields(row, readOnly) {
  const external = row.kind === "external"; const disabled = readOnly || !canEditModule("documentos") ? "disabled" : ""; const people = documentOptions(documentPeople(), row.owner || "");
  ...
  <div class="documents-grid"><label class="documents-field"><span class="documents-field-label">${external ? "Setor responsável" : "Elaborado por"}</span>${external ? `<input class="input-basic" name="sector" value="${escapeHtml(row.sector || "")}" ${disabled}>` : `<select class="input-basic" name="owner" ${disabled}>${people}</select>`}</label><label class="documents-field"><span class="documents-field-label">${external ? "Local de armazenamento" : "Setor"}</span>${external ? `<input class="input-basic" name="storageLocation" value="${escapeHtml(row.storageLocation || "")}" ${disabled}>` : `<input class="input-basic" name="sector" value="${escapeHtml(row.sector || "")}" ${disabled}>`}</label></div>
  ...
  <div class="documents-grid three"><label class="documents-field"><span class="documents-field-label">Setor da cópia</span><input class="input-basic" name="copySector" value="${escapeHtml(row.copySector || "")}" ${disabled}></label><label class="documents-field"><span class="documents-field-label">Responsável da cópia</span><select class="input-basic" name="copyOwner" ${disabled}>${documentOptions(documentPeople(), row.copyOwner || "")}</select></label><label class="documents-field"><span class="documents-field-label">Data da entrega</span><input class="input-basic" name="copyDate" type="date" value="${escapeHtml(row.copyDate || "")}" ${disabled}></label></div>
