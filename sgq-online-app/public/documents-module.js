...
function renderDocumentsCharts() {
  ...
  const make = (id, type, entries) => { const canvas = document.getElementById(id); if (!canvas) return; documentsCharts[id] = new Chart(canvas, { type, data: { labels: entries.map(([label]) => label), datasets: [{ data: entries.map(([, value]) => value), backgroundColor: colors, borderColor: "#0b2d50", borderWidth: 2, borderRadius: type === "bar" ? 5 : 0 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: "bottom", labels: { color: "#9db8d2", boxWidth: 11, font: { size: 10 } } } }, scales: type === "bar" ? { x: { ticks: { color: "#9db8d2", font: { size: 10 } }, grid: { color: "rgba(157,184,210,.12)" } }, y: { beginAtZero: true, ticks: { precision: 0, color: "#9db8d2" }, grid: { color: "rgba(157,184,210,.12)" } } } : {} } }); };
