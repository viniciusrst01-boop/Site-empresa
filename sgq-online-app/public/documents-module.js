...
function renderDocumentsModule() {
  ...
  pageContent.innerHTML = `
    ${moduleHeaderHtml("documentos", { category: "INFORMAÇÃO DOCUMENTADA · 7.5", description: "", actions: false })}
...
function documentsRegistrationHtml() {
  ...
  return `<section class="documents-registration"><header class="documents-panel-head">${moduleIcon("documentos")}<div><h2>Cadastro de documento</h2><p>Ao salvar, o documento entra na Lista Mestra aguardando aprovação.</p></div></header><form class="documents-form" data-doc-registration>
    <section class="documents-registration-section"><div class="documents-section-label">${moduleIcon("documentos")} Identificação</div>
    ...
    <div class="documents-grid documents-revision-grid"><label class="documents-field"><span class="documents-field-label">Tipo</span><select class="input-basic" name="type" data-doc-type ${disabled}>${documentOptions(documentTypes, "Procedimento")}</select></label><label class="documents-field" data-doc-other-type hidden><span class="documents-field-label">Especifique o tipo</span><input class="input-basic" name="otherType" ${disabled}></label>
    <label class="documents-field"><span class="documents-field-label">Revisão</span><input class="input-basic" name="version" placeholder="Ex.: 01" ${disabled}></label><label class="documents-field"><span class="documents-field-label">Data da revisão</span><input class="input-basic" name="revisionDate" type="date" ${disabled}></label><label class="documents-field"><span class="documents-field-label">Data da elaboração</span><input class="input-basic" name="elaborationDate" type="date" value="${documentToday()}" ${disabled}></label></div>
    ...
    </section><section class="documents-registration-section"><div class="documents-section-label">${moduleIcon("database")} Controle e retenção</div>
    ...
    </section><section class="documents-registration-section"><div class="documents-section-label">${moduleIcon("documentos")} Anexo e aprovação</div>
    ...
    </section><div class="documents-actions">${canEditModule("documentos") ? `<button class="documents-action-button" type="reset">${moduleIcon("trash")} Limpar</button><button class="documents-action-button primary" type="submit">${moduleIcon("save")} Salvar</button>` : `<span class="documents-approval-note">Seu perfil possui acesso somente para visualização.</span>`}</div>
  </form></section>`;
}
